...
scan() {
  this.logger.logTrace("ctrl.scan ...");
   this.mainDeviceconfig();
};

 mainDeviceconfig() {
  this.logger.logTrace("mainDeviceconfig()...");
  const regex = /^[^\\\/\:\*\?\"\<\>\|]+$/;
  ...
  if (regex.test(this.fileName)) {
    this.logger.logTrace("mainDeviceconfig() -> if (regex.test(fileName))");
    ...
    this.logger.logTrace("mainDeviceconfig() ELSE FOR if (regex.test(fileName))");
    let text = this.resourceString['SDE_CHARACTERS_CANNOT_BE'].replace('{0}', '\\ / : * ? " < > |');
    this.errorHandlerService.showErrorAlert(text, '', null, null); 
  }
}

deviceCallbackSuccess() {
  this.logger.logTrace("DeviceCallBack_Success -> respText:");
  this.getScanStatus();
}

 deviceCallBackFailure(respText, newresp) {
  this.logger.logException("DeviceCallBack_Failure -> respText:' + respText + ' newresp:' + newresp");
  //this.errorHandlerService.XBB_DEVICE_EIP_DEVICE_CONFIG_DISABLED();
   this.progress.close();
}

getScanStatus() {
  this.logger.logTrace("getScanStatus()...");
...
callback_success(reqText, respText) {
  this.logger.logTrace("getScanStatus() -> callback_success");
  this.getjobmamt();
}
callback_failure(respText, newresp) {
  this.logger.logException('callback_failure -> respText:' + respText + ' newresp:' + newresp);
  this.errorHandlerService.DEVICE_EIP_SCANV2_SERVICES_DISABLED();
  this.progress.close();
}

 getjobmamt() {
  this.logger.logTrace('getjobmanagementInterfaceVersion()...');
...
Jobcallback_success(reqText, respText) {
  this.logger.logTrace('Jobcallback_success()...');
  this.CheckTemplate();
}
Jobcallback_failure(reqText, respText) {
  this.logger.logException('Jobcallback_failure -> reqText:' + reqText + ' respText:' + respText);
  this.errorHandlerService.DEVICE_EIP_SCANV2_SERVICES_DISABLED();
  this.progress.close();
}
...
Templatecallback_success() {
  this.logger.logTrace('Templatecallback_success()...');
  this.selectedNote={
  ...
  this.logger.logTrace('Templatecallback_success() file name before:' + this.selectedNote.fileName);
  var values = this.scanOptionService.getValues(this.selectedNote);

  this.logger.logTrace('Templatecallback_success() values:' + values.fileName);
 ...
 Templatecallback_failure(respText, newresp) {
  this.logger.logException('Templatecallback_failure -> respText:' + respText + ' newresp:' + newresp);
  this.errorHandlerService.DEVICE_EIP_SCANV2_SERVICES_DISABLED();
}
