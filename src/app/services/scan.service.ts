...
export class ScanService {
  ...
  public callbacks = {
    ...
    handleBeginCheckFailure: (request: any, response: any) => {
      this.logService.logException(response);
      this.logService.logException(request);
    ...
    completeScan: (detail: any) => {
      this.logService.logException(detail);
  ...
  public scan(model): Promise<void> {
      this.progress = this.modalService.showProgressAlert('SDE_SCANNING1', '');
      this.logService.logTrace('service.scan');
      if (this.isScanning) {
        this.logService.logTrace('service.scan -> isScanning : Please wait!!!!');
        throw 'SDE_PLEASE_WAIT_UNTIL';
      }

      this.jobid = this.jobService.generateNewJobID();
      this.logService.logTrace('scanService => scan => jobID:' + this.jobid);
      ...
      return this.jobService.registerJob(model).then((result)=>{      
          ...
          this.logService.logTrace('scanService => scan => template:' + tStr);
          ...
          this.promiseReject = reject;
            ...
            this.logService.logTrace('service.scan -> calling putTemplate()');
  ...
  putTemplate(tStr): Promise<any> {
      ...
      this.logService.logTrace('putTemplate()...');
      const printerUrl =  this.env.apiUrl;
      const templateName= this.scanTemplate.name; //templateName
      //this.logService.logTrace("file name in scan Template :"+this.scanTemplate.name)
      function finish (callId: any, response: any) {
        //this.logService.logTrace('putTemplate => successCallback');
        this.logService.logTrace(`scanService => putTemplate => callId:${callId} response:${response}`);
        this.finishPutTemplate(callId, response,printerUrl);
        const result={};
        resolve (result);
      };
      function fail  (result: any)  {
        this.logService.logException(result);
  ...
  finishPutTemplate(callId: any, response: string, printerUrl: string,  timeoutInMinutes: number):Promise<any>{
      return new Promise((resolve,reject)=>{
        this.logService.logTrace(`finishPutTemplate(callId,response) -> callId: ${callId} response: ${response}`);
        const xmlDoc = xrxStringToDom(response);
        this.logService.logTrace(`finishPutTemplate(callId,response) -> xmlDoc: ${xmlDoc}`);
       ...
       async  function successCallback  (envelope: any, response: any)  {
          this.logService.logTrace(`function finish(callId, response) -> callId: ${callId} response: ${response}`);
          let responseJobId : string = xrxScanV2ParseInitiateScanJobWithTemplate(response);
          //this.logService.logTrace("response job Id : "+ responseJobId);
          this.scanTemplate.jobId = responseJobId;
          //this.logService.logTrace("response scan template job Id : "+ this.scanTemplate.jobId);
          // Let everyone know the job has been submitted.

          // Begin the check loop.
           this.startScanTime = new Date();
           this.stopScanTime = new Date();
           this.stopScanTime.setMinutes(this.stopScanTime.getMinutes() + timeoutInMinutes);
         await this.beginCheckLoop(this.scanTemplate.jobId);
        };
        function errorCallback  (env: any,message :any)  {
          this.logService.logException(`function fail(env, message) {  -> env: ${env} message: ${message}`);
  ...
  checkScanTimeout(): boolean {
    //this.logService.logTrace(`checkScanTimeout :${this.startScanTime}`);
  ...
  beginCheckLoop(jobid:string): Promise<any> { 
    ...
    this.logService.logTrace('beginCheckLoop()...');
    return new Promise((resolve,reject)=>{
      
      function checkLoop(request: any, response: any)  {
        this.logService.logTrace('checkLoop(request, response) -> request:' + request + ' response:' + response);
        let jobStateReason = '';
        let info = xrxJobMgmtParseGetJobDetails(response);
        //this.logService.logTrace("inside checkLoop => info  : "+info);
        let jobState = xrxGetElementValue(info, 'JobState');
        //this.logService.logTrace("inside checkLoop => jobState : "+jobState)
        const dummy = xrxJobMgmtParseJobStateReasons(response);
          this.logService.logTrace('checkLoop(request, response) -> jobState:' + jobState + ' dummy:' + dummy);
        if (jobState === null || jobState === 'Completed') {
          this.logService.logTrace('if (jobState === null || jobState === Completed)');
          jobStateReason = xrxParseJobStateReasons(response);
          this.logService.logTrace('jobStateReason:' + jobStateReason);
        }
        ...
        if (this.checkScanTimeout()) {
          this.logService.logTrace('if (checkScanTimeout()) ');
        ...
        if (jobState === 'Completed' && jobStateReason === 'JobCompletedSuccessfully') {
          ...
          this.logService.logTrace('if (jobState === Completed && jobStateReason == JobCompletedSuccessfully) { ');
        ...
        if (jobState === 'Completed' && jobStateReason === 'InputScanSizeNotDetermined') {
          this.logService.logTrace('if (jobState === Completed && jobStateReason === InputScanSizeNotDetermined) {  jobState:' + jobState + ' jobStateReason:' + jobStateReason);
        ...
        //   this.logService.logTrace('if (jobState === Completed && jobStateReason && jobStateReason != JobCompletedSuccessfully) {');
        //   this.broadcastJobState('jobProgress', 'Completed',jobStateReason);
        //   this.modalService.closeAllModals();
        //   this.errorHandlerService.APP_UNAVAILABLE_AT_THIS_TIME();
        //   return;
        // } else {
        //   this.logService.logTrace('jobProgress:' + jobState);
        ...
        if (jobState === 'Completed' && jobStateReason == 'JobCompletedSuccessfully') {
          ...
          return;
            ...
            this.logService.logTrace('else if (jobState === Completed && (jobStateReason === JobAborted || jobStateReason === AbortBySystem)) {')
            ...
            this.logService.logTrace('else if (jobState === Completed && (jobStateReason === JobCanceledByUser || jobStateReason === CancelByUser)) {');
            this.errorHandlerService.SDE_JOB_CANCELED1();
            ...
            this.logService.logTrace('else if ProcessingStopped NextOriginalWait');
            setTimeout(()=>{
            ...
            this.logService.logTrace('else if Completed ProcessingStopped');
            setTimeout(()=>{
              this.callbacks.handleJobComplete();
            },500
            );
            ...
            this.logService.logTrace('else if JobCanceledBUser');
            this.broadcastJobState('jobProgress', jobState,jobStateReason);
            this.callbacks.handleJobCanceled();
            setTimeout(()=>{
              this.deleteScanTemplate();
            ...
            this.logService.logTrace('else if (jobState === null && jobStateReason !== ) {  jobStateReason:');
            this.errorHandlerService.SDE_JOB_CANCELED1();
            ...
            setTimeout(()=>{
              //this.logService.logTrace("inside checkloop -> else case ->"+jobid);
              this.beginCheckLoop(jobid);
            },2000
            );
  ...
  success(message:any){
    //this.logService.logTrace("Scan Service Success CallBack Function ,  Message " + message);
  }

  failure(message: any) {
  this.logService.logException("Scan Service Failure CallBack Function , Error Message " + message);
  }
}
